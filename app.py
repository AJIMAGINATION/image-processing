...
def water_color_effect(decode_array_to_img):
    water_c = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)
    status, output_img = cv2.imencode('.PNG', water_c)

    return output_img

def invert(decode_array_to_img):
    invert_c = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG', invert_c)

    return output_img

def HDR(decode_array_to_img):
    hdr_c = cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.15)
    status, output_img = cv2.imencode('.PNG', hdr_c)
